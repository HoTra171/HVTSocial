...
export const requestResetOtp = async (req, res) => {
  try {
    ...
    await db
      ...
      .input('reset_otp_attempts', 0).query(`
        UPDATE users
        SET
          reset_otp = @reset_otp,
    ...
    return res.status(200).json({
      message: 'Mã OTP đã được gửi đến email của bạn (nếu email tồn tại trong hệ thống).',
    });
  } catch (error) {
...
export const resetPasswordWithOtp = async (req, res) => {
  try {
    ...
    if (!email || !otp || !newPassword) {
      return res.status(400).json({ message: 'Vui lòng nhập đủ email, OTP và mật khẩu mới' });
    }
    ...
    if (user.reset_otp_attempts >= MAX_ATTEMPTS) {
      return res.status(400).json({
        message: 'Bạn đã nhập sai OTP quá số lần cho phép. Vui lòng yêu cầu OTP mới.',
      });
    }
    ...
    if (
      ...
      await db
        .request()
        .input('id', user.id)
        .input('reset_otp_attempts', (user.reset_otp_attempts || 0) + 1).query(`
          UPDATE users
    ...
    await db.request().input('id', user.id).input('password', hashedPassword).query(`
        UPDATE users
        SET
          password = @password,
          reset_otp = NULL,
...
export const changePassword = async (req, res) => {
  try {
    ...
    // 4. Cập nhật vào DB
    await db.request().input('id', userId).input('password', hashedNewPassword).query(`
        UPDATE users
...
export const refreshToken = async (req, res) => {
  try {
    ...
    await db.request().input('id', storedToken.id).input('new_token', tokens.refreshToken).query(`
